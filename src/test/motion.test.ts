...
describe('Motion Detection System', () => {
  ...
  // let mockCanvas: HTMLCanvasElement // Reserved for future use
  // let mockContext: CanvasRenderingContext2D // Reserved for future use
  ...
  describe('Motion Calculation', () => {
    function calculateMotionIntensity(
  ...
  describe('Filter Frequency Mapping', () => {
    function mapMotionToFilterFreq(motionIntensity: number): number {
      const minFreq = 200;
      const maxFreq = 3200;
      // Clamp motion intensity to 0-1 range
      const clampedMotion = Math.max(0, Math.min(1, motionIntensity));
      return minFreq + clampedMotion * (maxFreq - minFreq);
    }
    ...
    it('clamps values within expected range', () => {
      expect(mapMotionToFilterFreq(-0.1)).toBe(200); // Below min should clamp to 0, resulting in minFreq = 200
      expect(mapMotionToFilterFreq(1.1)).toBe(3200); // Above max should clamp to 1, resulting in maxFreq = 3200
    });
  ...
  describe('Performance Optimization', () => {
    ...
    it('handles high motion intensity without performance degradation', () => {
      ...
      for (let i = 0; i < largeFrame.length; i += 4) {
        const diff =
          Math.abs(largeFrame[i] - previousFrame[i]) +
